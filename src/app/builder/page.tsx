      ...
      {payPromptOpen && (
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', boxShadow: '0 12px 50px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted2)', textTransform: 'uppercase', letterSpacing: '.08em', margin: '2px 0 10px' }}>Making a lot of shows?</div>

            {/* Yearly — best value */}
            <button
              onClick={() => checkoutSubscription('yearly')}
              disabled={!!payBusy}
              style={{ width: '100%', textAlign: 'left', padding: '11px 14px', marginBottom: 8, borderRadius: 10, background: 'rgba(232,64,74,0.1)', border: '1px solid rgba(232,64,74,0.4)', color: 'var(--text)', cursor: payBusy ? 'default' : 'pointer', opacity: payBusy && payBusy !== 'yearly' ? 0.5 : 1 }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: 700, fontSize: 14 }}>{payBusy === 'yearly' ? 'Redirecting…' : 'Go Unlimited — $59.99/yr'}</span>
                <span style={{ fontSize: 10, fontWeight: 700, background: 'rgba(232,64,74,0.25)', padding: '2px 7px', borderRadius: 20 }}>BEST VALUE</span>
              </div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>Unlimited exports · save 28%</div>
            </button>
            ...
            <button
              ...
              disabled={!!payBusy}
              style={{ width: '100%', textAlign: 'left', padding: '11px 14px', borderRadius: 10, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', cursor: payBusy ? 'default' : 'pointer', opacity: payBusy && payBusy !== 'monthly' ? 0.5 : 1 }}
            >
